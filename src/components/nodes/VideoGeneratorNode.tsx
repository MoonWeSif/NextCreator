...
import type {
  VideoGeneratorNode as VideoGeneratorNodeType,
  VideoGeneratorNodeData,
} from "./videoGeneratorConfig";
import {
  getVideoApiProtocolConfig,
  getVideoGeneratorParameterLabels,
...
function getStatusLabel(status: VideoGeneratorNodeData["status"]) {
  switch (status) {
    case "loading":
      return "Running";
    case "success":
...
function getOutputLabel(data: VideoGeneratorNodeData) {
  if (data.outputVideo || data.videoData) return "1 个视频";
  if (data.taskId) return "任务完成";
  return "已完成";
}

function VideoGeneratorNodeBase({ id, data, selected }: NodeProps<VideoGeneratorNodeType>) {
  ...
  const modelLabel = getVideoModelDisplayName(data);
  const parameterLabels = getVideoGeneratorParameterLabels(data);
  const { handleGenerate, validationError } = useVideoGeneratorExecution(id, data);
  const {
    isPromptConnected,
    promptText,
    hasEmptyImageInputs,
  ...
  const canRun = hasResolvedPrompt && data.status !== "loading" && !validationError;
      ...
      >
        <div className="nc-image-info-header">
          <div className="flex min-w-0 items-center gap-2.5">
            ...
            <span className="truncate text-[15px] font-semibold">{data.label || "视频生成"}</span>
          </div>
          <div className="flex flex-shrink-0 items-center gap-1.5">
            {!hasResolvedPrompt && <CircleAlert className="w-4 h-4 text-warning" />}
            {isPromptConnected && hasEmptyImageInputs && <AlertTriangle className="w-4 h-4 text-warning" />}
            {data.status === "loading" && <Loader2 className="w-4 h-4 animate-spin text-info" />}
            {(data.outputVideo || data.videoData) && <Film className="w-4 h-4 text-success" />}
            {!isOverlay && (
              <button
                type="button"
            ...
            >
              ...
              {data.status === "loading" && (
                <span className="min-w-0 flex-1 truncate text-xs opacity-75">
                  {data.taskStage === "queued" ? "排队中" : `生成中 ${data.progress || 0}%`}
                </span>
              )}
...
function ParameterInfoRow({ labels }: { labels: string[] }) {
  return (
    <div className="flex min-w-0 items-start gap-2">
...
function InputInfoRow({ sources }: { sources: ConnectedInputSource[] }) {
  return (
    <div className="flex min-w-0 items-start gap-2">
...
export const VideoGeneratorNode = memo(VideoGeneratorNodeBase);
